...
{
	//-------------------------------------------------------------------------
	public class SteamVRTeleport_NVR : MonoBehaviour
	{
		...
		private NVRHand pointerHand = null;
		...
		//-------------------------------------------------
		private static SteamVRTeleport_NVR _instance;
		public static SteamVRTeleport_NVR instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = GameObject.FindObjectOfType<SteamVRTeleport_NVR>();
				}
        ...
        private Vector3 FeetPositionEstimate()
        {
            return new Vector3(NVRPlayer.Instance.Head.transform.position.x, NVRPlayer.Instance.gameObject.transform.position.y, NVRPlayer.Instance.Head.transform.position.z);
        }
		...
		void Update()
		{
			...
			else
			{
				...
				{
					if ( newPointerHand == null && !pointerHand.Inputs[NVRButtons.Touchpad].IsPressed )
					{
						//Hide the pointer
						HidePointer();
					}
		...
		private void UpdatePointer()
		{
			...
			if ( hitTeleportMarker != null ) //Hit a teleport marker
			{
				...
				if ( showPlayAreaMarker )
				{
					...
					TeleportArea_NVR teleportArea = pointedAtTeleportMarker as TeleportArea_NVR;
